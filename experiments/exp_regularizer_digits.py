"""Real-data JEPA-style regularizer experiment on sklearn digits."""
...
class DigitJEPA(torch.nn.Module):
...
def load_digit_halves(seed: int, test_size: float) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    digits = load_digits()
    images = digits.images.astype(np.float32) / 16.0
    # Context is the left half of the digit; target is the right half.
...
def train_one(params: dict[str, Any]) -> dict[str, Any]:
    ...
    x_train, x_test, y_train, y_test = load_digit_halves(cfg.seed, cfg.test_size)
    ...
    z_np = test_z.numpy()
    p_np = test_p.numpy()
    z_dir_np = z_dirs.numpy()
    p_dir_np = p_dirs.numpy()
    z_metrics = covariance_metrics(z_np)
    p_metrics = covariance_metrics(p_np)
    return {
        "experiment": "regularizer_digits",
        "method": cfg.method,
        "seed": cfg.seed,
        "epochs": cfg.epochs,
...
if __name__ == "__main__":
    args = parse_args(
        ...
        ROOT / "results" / "exp_regularizer_digits.csv",
    )
